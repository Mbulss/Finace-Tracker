...
interface EditTransactionModalProps {
  ...
  onClose: () => void;
  onSave: (id: string, data: { type: TransactionType; amount: number; category: string; note: string }) => void;
}
...
export function EditTransactionModal({ transaction, onClose, onSave }: EditTransactionModalProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    onSave(transaction.id, { type, amount: num, category: category || categories[0], note: note.trim() });
    setLoading(false);
    onClose();
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-xl border border-border dark:border-slate-600 py-3 font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700"
            >
              Batal
            </button>
            <button
