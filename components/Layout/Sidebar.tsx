...
export function Sidebar({ isOpen = true, onClose }: SidebarProps) {
  ...
  const [botOpening, setBotOpening] = useState(false);
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const url = res.ok && data?.code ? `${TELEGRAM_BOT_URL}?start=${data.code}` : TELEGRAM_BOT_URL;
                  openTelegramUrl(url);
                } catch {
              ...
              disabled={botOpening}
              className="flex min-h-[44px] w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium text-muted transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-700 dark:text-slate-400 dark:hover:text-slate-200 disabled:opacity-70"
            >
              ...
              {botOpening ? "Memuat..." : `Buka Bot (@${TELEGRAM_BOT_USERNAME})`}
            </button>
            <Link
              href="/faq"
              onClick={onClose}
