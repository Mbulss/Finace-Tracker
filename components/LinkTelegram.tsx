    ...
    <section id="link-telegram" className="rounded-2xl border-2 border-primary/20 dark:border-primary/30 bg-primary/5 dark:bg-primary/10 p-4 shadow-card sm:p-6">
      ...
      {!code ? (
        ...
        <div className="rounded-xl border border-border dark:border-slate-600 bg-slate-50 dark:bg-slate-800 p-4">
          <p className="mb-3 text-sm font-medium text-slate-700 dark:text-slate-200">Link otomatis (paling gampang)</p>
          <a
            href={`${TELEGRAM_BOT_URL}?start=${code}`}
            target="_blank"
            rel="noopener noreferrer"
            className="mb-4 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-primary-hover dark:bg-sky-600 dark:hover:bg-sky-500"
          >
          ...
          <p className="mb-2 text-sm text-muted dark:text-slate-400">Atau kirim manual ke bot:</p>
