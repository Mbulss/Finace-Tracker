...
export function EmailSyncManager() {
  ...
  const fetchIntegration = useCallback(async (session: any) => {
    if (!session) { setProviderToken(null); setIsIntegrated(false); return; }
  ...
  async function handleSync() {
    if (!isIntegrated && !providerToken) return;
    setLoading(true);
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 items-stretch">
        ...
        <div className="lg:col-span-8 flex flex-col h-full animate-fade-in-up [animation-delay:200ms]">
           <section className="h-full bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl p-8 sm:p-10 relative overflow-hidden flex flex-col gap-6 hover:-translate-y-2 hover:shadow-2xl transition-all duration-300">
              ...
              <div className={`rounded-3xl p-8 flex flex-col md:flex-row items-center gap-8 transition-all border-2 ${isIntegrated ? "bg-emerald-50/30 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/20 shadow-sm" : "bg-slate-50 dark:bg-slate-800/30 border-slate-100 dark:border-slate-800 shadow-sm"}`}>
                 <div className={`w-24 h-24 rounded-[2rem] flex items-center justify-center shrink-0 shadow-lg ${isIntegrated ? "bg-emerald-500 text-white animate-pulse-subtle" : "bg-white dark:bg-slate-700 text-slate-300 dark:text-slate-500 ring-1 ring-slate-100 dark:ring-slate-600"}`}>
                    <svg className="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                       {isIntegrated ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />}
                    </svg>
                 </div>
                 <div className="text-center md:text-left flex-1 space-y-2">
                    <div className="flex items-center justify-center md:justify-start gap-2">
                       <span className={`w-2.5 h-2.5 rounded-full ${isIntegrated ? "bg-emerald-500 animate-pulse" : "bg-slate-300 dark:bg-slate-600"}`} />
                       <span className={`text-xs font-black uppercase tracking-widest ${isIntegrated ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}>
                          {isIntegrated ? "System Active" : "Waiting Setup"}
                       </span>
                    </div>
                    <h3 className="text-2xl font-black text-slate-900 dark:text-white">
                       {isIntegrated ? "Sinkronisasi Berjalan" : "Gmail Belum Terhubung"}
                    </h3>
                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400 leading-relaxed max-w-sm mx-auto md:mx-0">
                       {isIntegrated ? "Kami siap mengolah resi bank terbaru di inbox Anda kapan saja." : "Hubungkan akun Google Anda untuk mengaktifkan pelacakan pengeluaran otomatis."}
                    </p>
                 </div>
              </div>
